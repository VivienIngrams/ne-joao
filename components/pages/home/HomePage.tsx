...
export function HomePage({ data, encodeDataAttribute }: HomePageProps) {
  return (
    <section className=" h-screen w-screen mx-auto ml-[200px]">
      ...
      <div className=" flex flex-col py-8 w-1/3">
     
      <h1 className="font-barlowC font-thin text-8xl text-red-700 align-left">
          LabIO
        </h1> {data?.overview && (
          <h3 className='  text-2xl   text-white '> {data?.overview}</h3>
        )}</div>
      </div>
    </section>
  )
